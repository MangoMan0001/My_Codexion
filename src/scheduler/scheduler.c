...
/*   Updated: 2026/02/15 06:59:55 by ayhirose         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int compare_coders(int id1, int id2, t_rules *rule)
{
	long long time1;
	long long time2;
	...
	else // EDFなら最後のコンパイル時間
	{
		...
		if (time1 == time2)// 時間が同じなら、先に並んだ人(enqueue_time)を優先する！
        {
            time1 = rule->coders[id1 - 1].enqueue_time;
            time2 = rule->coders[id2 - 1].enqueue_time;
        }
...
// nがヒープ構造状最適な位置へと上る
static void	shift_up(int n, t_rules *rule)
{
